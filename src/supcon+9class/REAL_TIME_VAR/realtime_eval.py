...
if os.getenv("MJKAN_FROM_HF") == "1":
    CKPT=hf_hub_download(HF,"realtime_joint_temporal/rtjt_best.pt")
else:
    CKPT=str(Path(OUT,"rtjt_best.pt"))
...
print(f"[REALTIME] loaded | N_CTX={N_CTX}")
...
print(f"\n{'='*55}\nREALTIME — EVAL SCORECARD (temporal split)\n{'='*55}")
print(f"  Overall:  {ov:.3f} | QUIC: {qa:.3f} | TLS: {ta:.3f}  <-- honest protocol transfer")
...
try:
    ...
    plt.title(f"REALTIME — QUIC ○ / TLS △ | TLS={ta:.3f} inter={inter:.3f} (temporal split)")
    plt.legend(fontsize=7,ncol=2); plt.xticks([]); plt.yticks([])
    plt.tight_layout(); plt.savefig(str(Path(OUT,"realtime_tsne.png")),dpi=130,bbox_inches="tight")
    print(f"[saved] {OUT}/realtime_tsne.png")
except Exception as e: print("[tsne skipped]",e)

result={"model":"realtime","overall":float(ov),"quic":float(qa),"tls":float(ta),
...
Path(f"{OUT}/realtime_eval.json").write_text(json.dumps(result,indent=2))
print(f"[saved] {OUT}/realtime_eval.json")
