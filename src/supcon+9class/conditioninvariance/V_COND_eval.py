...
OUT=os.path.join(os.getenv("OUT_DIR","./checkpoints"),"v_cond"); Path(OUT).mkdir(parents=True,exist_ok=True)

if os.getenv("MJKAN_FROM_HF") == "1":
    CKPT=hf_hub_download(HF,"condition_invariance/v_cond/v_cond_best.pt")
else:
    CKPT=str(Path(OUT,"v_cond_best.pt"))
...
class GatedFiLM(nn.Module):
    def __init__(s,n_ctx,dm,hidden=64):
        ...
        s.gate=nn.Sequential(nn.Linear(n_ctx,hidden),nn.ReLU(),nn.Linear(hidden,dm),nn.Sigmoid()); s.dm=dm
        nn.init.zeros_(s.net[-1].weight); nn.init.zeros_(s.net[-1].bias)
    def forward(s,h,ctx):
        gb=s.net(ctx); gamma,beta=gb[:,:s.dm],gb[:,s.dm:]
        g=s.gate(ctx); return g*((1.0+gamma)*h+beta)+(1.0-g)*h
...
print(f"[V-COND] loaded | QUIC={ck.get('quic',0):.3f} TLS={ck.get('tls',0):.3f} perturbed={ck.get('perturbed_acc',0):.3f}")
...
d5=np.load(hf_hub_download(HF,"protocol_invariance/data/fiveg_youtube.npz"),allow_pickle=True)
...
if "ctx_mean" in meta["norm"]:
    cm=np.array(meta["norm"]["ctx_mean"],np.float32); cs=np.array(meta["norm"]["ctx_std"],np.float32)
    fiveg_ctx=((fiveg_ctx-cm)/cs).astype(np.float32)

# condition augmentation (same as training — used for robustness eval)
def condition_augment_heavy(seq_np):
    seq=torch.from_numpy(seq_np).float(); B=seq.shape[0]; out=seq.clone()
    real=(out.abs().sum(-1)>0).float()
    lat=torch.empty(B,1).uniform_(0.7,1.5); out[:,:,2]=out[:,:,2]*lat*real+out[:,:,2]*(1-real)
    noise=torch.randn(B,30)*0.10
    for tt in range(1,30): noise[:,tt]=0.6*noise[:,tt-1]+0.4*noise[:,tt]
    out[:,:,2]=out[:,:,2]+noise*real
    return out.numpy()
...
# robustness: perturbed CESNET (the clean condition-invariance metric)
pert_seq=condition_augment_heavy(te_seq)
zpe=embed(pert_seq,te_ctx); pp=knn(ztr[ri],tr_y[ri],zpe); pert_acc=float((pp==te_y).mean())

# 5G YouTube transfer
p5=knn(ztr[ri],tr_y[ri],z5); vote_vid=float((p5==VS).mean())
...
print(f"\n{'='*55}\nV-COND — EVAL SCORECARD\n{'='*55}")
print(f"  Overall:  {ov:.3f} | QUIC: {qa:.3f} | TLS: {ta:.3f}")
print(f"  Intra: {intra:.3f} (target >0.7) | Inter: {inter:.3f} (target <0.3)")
print(f"  Perturbed-CESNET acc: {pert_acc:.3f}  <-- condition-invariance metric")
print(f"  5G YouTube: vote->video={vote_vid:.1%} cos_median={cos_med:.3f} cos>0.7={cos_gt:.1%}")
...
try:
    ...
    plt.title(f"V-COND — QUIC ○ / TLS △ | TLS={ta:.3f} perturbed={pert_acc:.3f}")
    plt.legend(fontsize=7,ncol=2); plt.xticks([]); plt.yticks([])
    plt.tight_layout(); plt.savefig(str(Path(OUT,"v_cond_tsne.png")),dpi=130,bbox_inches="tight")

    # 5G t-SNE overlay
    ids2=np.concatenate([rng.choice(np.where(te_y==c)[0],min(120,(te_y==c).sum()),replace=False) for c in range(NC)])
    allp=np.concatenate([zte[ids2],z5]); e2=TSNE(2,perplexity=30,init="pca",random_state=0).fit_transform(allp)
    ec,e5=e2[:len(ids2)],e2[len(ids2):]; yc=te_y[ids2]
    plt.figure(figsize=(10,8))
    for c in range(NC):
        mc=yc==c; plt.scatter(ec[mc,0],ec[mc,1],s=10,alpha=0.35,color=cmap(c%10),label=CATS[c] if c==VS else None)
    plt.scatter(e5[:,0],e5[:,1],s=35,marker="*",color="red",edgecolors="k",linewidths=0.3,label="5G YouTube")
    plt.title(f"V-COND — 5G YouTube (★) | vote->video={vote_vid:.0%} cos={cos_med:.2f}")
    plt.legend(fontsize=8); plt.xticks([]); plt.yticks([])
    plt.tight_layout(); plt.savefig(str(Path(OUT,"v_cond_tsne_5g.png")),dpi=130,bbox_inches="tight")
    print(f"[saved] t-SNE plots in {OUT}/")
except Exception as e: print("[tsne skipped]",e)

result={"model":"V-COND","overall":float(ov),"quic":float(qa),"tls":float(ta),
        "intra":float(intra),"inter":float(inter),"latency_ms":float(np.median(ts)),
        "perturbed_cesnet_acc":pert_acc,
        "fiveg":{"vote_video":vote_vid,"cos_median":cos_med,"cos_gt07":cos_gt},
        "per_class_tls":{CATS[c]:float((pred[t&(te_y==c)]==c).mean()) for c in range(NC) if (t&(te_y==c)).sum()}}
Path(f"{OUT}/v_cond_eval.json").write_text(json.dumps(result,indent=2))
print(f"[saved] {OUT}/v_cond_eval.json")
