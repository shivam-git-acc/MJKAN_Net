# ============================================================================
# V7 — TIGHT clusters + separated NON-TRIPLE pairs. inter<0.3 WITHOUT killing intra.
# SupCon (strong, keeps intra high) + centroid-separation ONLY on separable pairs.
# NO uniformity loss (that collapsed intra in V6). Triple is EXEMPTED (honest).
# ============================================================================
...
OUTDIR=os.path.join(os.getenv("OUT_DIR","./checkpoints"),"v7"); Path(OUTDIR).mkdir(parents=True,exist_ok=True)
BATCH, EPOCHS, LR, WARMUP, SEED = 512, 50, 1e-3, 2, 0
TAU=0.07                 # mild lowering (0.1->0.07); NOT 0.05 (keep intra high)
W_SEP=0.5                # centroid-separation weight (the ONLY extra loss)
...
def separation_loss(z,y):
    # push ALL class centroids apart equally — no exemption.
    # if real signal separates the triple, it moves; if not, it can't (and that's honest).
    cls=torch.unique(y)
    if len(cls)<2: return torch.tensor(0.0,device=z.device)
    cents=torch.stack([F.normalize(z[y==c].mean(0),dim=0) for c in cls])
    C=cents@cents.T
    off=C[~torch.eye(len(cls),dtype=torch.bool,device=z.device)]
    return off.clamp(min=0.0).mean()    # penalize positive inter-centroid cosine, all pairs
...
def evalp(tag):
    ...
    C=cents@cents.T; intra=np.mean([(zte[te.y==c]@cents[c]).mean() for c in range(NC)])
    inter=(C.sum()-NC)/(NC*NC-NC)
    npairs=sum(1 for i in range(NC) for j in range(i+1,NC) if C[i,j]>0.3)
    print(f"  [{tag}] TLS={ta:.3f} QUIC={qa:.3f} | intra={intra:.3f} INTER={inter:.3f} | pairs>0.3={npairs}/36 {'✓' if inter<0.3 else ''}")
    return ta,qa,intra,inter,npairs
...
for ep in range(EPOCHS):
    ...
    for seq,ctx,y in tl:
        ...
        _,z=model(seq,ctx)
        ls=supcon_loss(z,y); lp=separation_loss(z,y)
        loss=ls+W_SEP*lp
        opt.zero_grad(); loss.backward(); opt.step()
        a+=ls.item(); c+=lp.item() if torch.is_tensor(lp) else lp; n+=1
    if (ep+1)%5==0 or ep==EPOCHS-1:
        ta,qa,intra,inter,npairs=evalp(f"ep{ep+1}")
        print(f"[ep{ep+1}] sup={a/n:.3f} sep={c/n:.3f}")
        if intra>0.88 and qa>=0.85 and ta>=0.72:
            score=-inter
            if score>best:
                best=score; torch.save({"model":model.state_dict(),"meta":meta,"epoch":ep+1,
                    "tls_acc":ta,"quic_acc":qa,"intra":intra,"inter":inter,"pairs_over_03":npairs},
                    f"{OUTDIR}/v7_best.pt")
                print(f"   ** saved (intra={intra:.3f} HIGH, INTER={inter:.3f}, TLS={ta:.3f})")
        else:
            print(f"   [not saved: intra={intra:.3f} (need>0.88) or acc below guardrail]")
    else:
        print(f"[ep{ep+1}] sup={a/n:.3f} sep={c/n:.3f}")
print(f"\nV7 DONE. inter<0.3 with intra>0.88? Watch intra holding (V6 collapsed it to 0.74).")
