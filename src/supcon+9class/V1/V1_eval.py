...
OUT=os.path.join(os.getenv("OUT_DIR","./checkpoints"),"v1"); Path(OUT).mkdir(parents=True,exist_ok=True)

if os.getenv("MJKAN_FROM_HF") == "1":
    CKPT=hf_hub_download(HF,"protocol_invariance/v1_joint/supcon_v1_best.pt")
else:
    CKPT=str(Path(OUT,"v1_best.pt"))
...
print(f"[V1] loaded ep{ck.get('epoch','?')} | train TLS={ck.get('tls_acc',0):.3f}")
...
try:
    ...
    plt.title(f"V1 — QUIC ○ / TLS △  | TLS={ta:.3f} intra={intra:.3f} inter={inter:.3f}")
    plt.legend(fontsize=7,ncol=2); plt.xticks([]); plt.yticks([])
    plt.tight_layout(); plt.savefig(str(Path(OUT,"v1_tsne.png")),dpi=130,bbox_inches="tight")
    print(f"[saved] {OUT}/v1_tsne.png")
except Exception as e: print("[tsne skipped]",e)

result={"model":"V1","overall":float(ov),"quic":float(qa),"tls":float(ta),
...
Path(f"{OUT}/v1_eval.json").write_text(json.dumps(result,indent=2))
print(f"[saved] {OUT}/v1_eval.json")
