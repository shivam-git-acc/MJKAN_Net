# ============================================================================
# SHIGAN / MJKAN-Net — RUNG 3 (Cell 1): SUPERVISED CONTRASTIVE  [FIXED]
# Changes from before: BatchNorm projection head (better spread), LR=1e-3 with
# 2-epoch warmup then constant (cosine-to-zero was freezing it), tau=0.1.
# Saves encoder. Pushes to baseline+supcon/ on HF.
# ============================================================================
import json, time, os
...
meta=json.loads(Path(hf_hub_download(HF_REPO,f"{HF_DATA}/pool_meta.json")).read_text()); NC=meta["num_classes"]; CATS=meta["categories"]
print("[device]",DEVICE,"| classes:",CATS)

class DS(Dataset):
    def __init__(s,split):
        d=np.load(hf_hub_download(HF_REPO,f"{HF_DATA}/pool_{split}.npz")); s.seq=torch.from_numpy(d["seq"]).float(); s.y=torch.from_numpy(d["label"]).long()
...
class SupConModel(nn.Module):
    def __init__(s,nf=5,dm=128,nl=2,ds=16,dp=0.1):
        ...
        # FIXED projection head: BatchNorm prevents poor spread
...
# warmup then constant (NO cosine-to-zero)
...
def embed_h(arr):
    ...
    for i in range(0,len(arr),1024):
        ...
        out.append(F.normalize(model(xb)[0],dim=1).cpu().numpy())
    return np.concatenate(out)
def knn_acc():
    ref=embed_h(tr_seq); q=embed_h(val_seq)
